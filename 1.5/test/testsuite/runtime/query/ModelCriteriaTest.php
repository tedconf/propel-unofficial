...
//		BookstoreDataPopulator::populate($con);
		...
		$subQuery = new BookQuery();
		$subQuery->addSelfSelectColumns();
		$subQuery->filterByTitle('%Php%', Criteria::LIKE);
		$subQuery->join('Review');
		
		$subQuery->setSubQueryAlias('b');
		...
		$expectedSql =
			...
			'SELECT author.ID, author.FIRST_NAME, author.LAST_NAME, author.EMAIL, author.AGE '.
			'FROM `author` '.
			'LEFT JOIN ('.
	...
	public function testJoinComposite()
	{
		...
		$sql = 'SELECT  FROM `reader_favorite` INNER JOIN book_opinion ON ((reader_favorite.BOOK_ID=book_opinion.BOOK_ID AND reader_favorite.READER_ID=book_opinion.READER_ID))'; // TODO: maybe we want to avoid double parentheses 
		$params = array();
	...
	public function testMergeWithJoins()
	{
		...
		$this->assertEquals('LEFT JOIN author a ON (book.AUTHOR_ID=a.ID) tableMap: null relationMap: Author previousJoin: null', $joins['a']->__toString(), 'mergeWith() does not remove an existing join');
		$c1 = new ModelCriteria('bookstore', 'Book', 'b');
		...
		$this->assertEquals('LEFT JOIN author a ON (book.AUTHOR_ID=a.ID) tableMap: null relationMap: Author previousJoin: null', $joins['a']->__toString(), 'mergeWith() merge joins to an empty join');
		...
		$this->assertEquals('LEFT JOIN author a ON (book.AUTHOR_ID=a.ID) tableMap: null relationMap: Author previousJoin: null', $joins['a']->__toString(), 'mergeWith() merge joins to an empty join');
		$this->assertEquals('INNER JOIN publisher p ON (book.PUBLISHER_ID=p.ID) tableMap: null relationMap: Publisher previousJoin: null', $joins['p']->__toString(), 'mergeWith() merge joins to an empty join');
	}
	...
	public function testMergeWithWiths()
	{
		...
		$this->assertEquals('LEFT JOIN author a ON (book.AUTHOR_ID=a.ID) tableMap: AuthorTableMap relationMap: Author previousJoin: null', $with['a']->__toString(), 'mergeWith() does not remove an existing join');
		...
		$this->assertEquals('LEFT JOIN author a ON (book.AUTHOR_ID=a.ID) tableMap: AuthorTableMap relationMap: Author previousJoin: null', $with['a']->__toString(), 'mergeWith() merge joins to an empty join');
		...
		$this->assertEquals('LEFT JOIN author a ON (book.AUTHOR_ID=a.ID) tableMap: AuthorTableMap relationMap: Author previousJoin: null', $with['a']->__toString(), 'mergeWith() merge joins to an empty join');
		$this->assertEquals('INNER JOIN publisher p ON (book.PUBLISHER_ID=p.ID) tableMap: PublisherTableMap relationMap: Publisher previousJoin: null', $with['p']->__toString(), 'mergeWith() merge joins to an empty join');
