...
class CriteriaTest extends BaseTestCase
{
	...
	public function testInjectionRiskAddJoin()
	{
		$this->c->add('myTable1.column1', '1 OR 1=1', Criteria::EQUAL);
		
		try {
			$this->c->addJoin('myTable1.column2', 'myTable2.column1)OR(2=2', Criteria::INNER_JOIN);
		
			$this->fail('throw Exception when no valid fieldname provided');
		} catch (PropelException $e) {
			$this->assertEquals("Invalid fieldname: myTable2.column1)OR(2=2. if you want to add constraints please use CriterionJoin.", $e->getMessage(), 'throw Exception when no valid fieldname provided');
		}
		
		$params = array();
		$result = BasePeer::createSelectSql($this->c, $params);

		$expect = "SELECT  FROM myTable1 WHERE myTable1.column1=:p1";
		$expect_params = array(
			array('table' => 'myTable1', 'column' => 'column1', 'value' => '1 OR 1=1'),
		);

		$this->assertEquals($expect, $result, 'addJoin() called with an injection risk-value');
		$this->assertEquals($expect_params, $params, 'addJoin() called with an injection risk-value');
	}  
	
	public function testInjectionRiskAddMultipleJoin()
	{
		$this->c->add('myTable1.column1', '1 OR 1=1', Criteria::EQUAL);
		
		try {
			$this->c->addMultipleJoin(
				array(
					array('myTable1.column2', 'myTable2.column1'),
					array('myTable1.column3', '3 OR 3=3')
				),
				Criteria::INNER_JOIN
			);
			...
			$this->assertEquals("Invalid fieldname: 3 OR 3=3. if you want to add constraints please use CriterionJoin.", $e->getMessage(), 'throw Exception when no valid fieldname provided');
		}
		...
		$this->assertEquals($expect, $result, 'addMultipleJoin() called with an injection risk-value');
		$this->assertEquals($expect_params, $params, 'addMultipleJoin() called with an injection risk-value');
	}
	...
	public function testBoolean()
	{
		...
		$expect_params = array(
		  array('table' => 'TABLE', 'column' => 'COLUMN', 'value' => true),
		);
		try {
			$params = array();
			$result = BasePeer::createSelectSql($this->c, $params);
	...
	public function testAddJoinArrayWithValue()
	{
		$c = new Criteria();
		$c->clearSelectColumns()->
			addJoin(
				array('TABLE_A.B_ID', 'TABLE_B.FIELD'), 
				array('TABLE_B.ID', 'some value'), 
				Criteria::LEFT_JOIN,
				array(false, true)
			)->
			addSelectColumn("TABLE_A.ID");
		...
		$expect = 'SELECT TABLE_A.ID FROM TABLE_A'
							.' LEFT JOIN TABLE_B ON (TABLE_A.B_ID=TABLE_B.ID AND TABLE_B.FIELD=:p1)';
		$expected_params = array('some value');
