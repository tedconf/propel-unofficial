...
class BasePeer
{
	...
	public static function createSelectSql(Criteria $criteria, &$params)
	{
		...
		$subQueryCriteria = $criteria->getSelectQueries();
		...
		foreach ($criteria->getJoins() as $join) {
			if ($join instanceof CriterionJoin) {
				$joinTables[] = $join->getRightTableAndAlias($params);
				$joinClause[] = $join->getClause($params);
				if (!$fromClause) {
					$fromClause[] = $join->getLeftTableAndAlias();
				}
			...
			foreach ($join->getConditions() as $index => $conditionDesc) {
				if ($ignoreCase) {
					if (!$conditionDesc['rightIsValue']) {
					 $condition .= $db->ignoreCase($conditionDesc['left']) . $conditionDesc['operator'] . $db->ignoreCase($conditionDesc['right']);
					} else {
						$params[] = $conditionDesc['right'];
						$paramPlace = ':p'.count($params);
						
						$condition .= $db->ignoreCase($conditionDesc['left']) . $conditionDesc['operator'] . $db->ignoreCase($paramPlace);
					}
				} else {
					if (!$conditionDesc['rightIsValue']) {
					 $condition .= $conditionDesc['left'] . $conditionDesc['operator'] . $conditionDesc['right'];
						...
						$condition .= $conditionDesc['left'] . $conditionDesc['operator'] . $paramPlace;
					}
