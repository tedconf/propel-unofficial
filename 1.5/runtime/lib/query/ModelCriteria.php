...
class ModelCriteria extends Criteria
{
	...
	/**
	 * Sets the alias for this query when using it as a subQuery
	 *
	...
	/**
	 * Returns the alias for this query when used as a subQuery
	 *
	 * @return    string The alias
	 */
	public function getSubQueryAlias()
	{
	 ...
	 * </code>
	 * 
	 * @param      mixed $relation string Relation to use for the join or ModelCriteria to use subQuery as join
	 * @param      string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
	 * @param      Criterion $extraJoinCriterion extra joinCriterion to use add with AND to the join-condition
	 *
	 * @return     ModelCriteria The current object, for fluid interface
	...
	public function join($relation, $joinType = Criteria::INNER_JOIN, Criterion $extraJoinCriterion = null)
	{
		...
		if ($relation instanceof ModelCriteria)
		{
			...
			$leftTableName = $this->getModelName();
			$leftName = $this->getModelAliasOrName();
			$leftTableAlias = isset($this->aliases[$leftName]) ? $leftName : null;
			
			// create a CriterionJoin object for this join
  		  	$join = new CriterionJoin();
  	  		$join->setJoinType($joinType);
  	  		if (null !== $previousJoin) {
  			  	$join->setPreviousJoin($previousJoin);
  		  	}
  	  		$join->setLeftTableName($leftTableName, $leftTableAlias);
  	  		$join->setRightTableName($relation, $relationAlias);
  	  		
            $join->setTableMap($relation->getTableMap());
  		
  	  		// add the SubQueryJoin to the current object
  	  		$this->addAlias($relationAlias, $relation->getTableMap()->getName());
			...
			$relationMap = $tableMap->getRelation($relationName);
  	  		...
  	  		$join->setRelationMap($this, $relationMap, $leftTableAlias, $relationAlias, $extraJoinCriterion);
	 ...
	 * @param boolean  $addAliasAndSelectColumns bool define if you want to automatically add the aliased select columns 
	 *
	 * @return Criteria this modified Criteria object (Fluid API)
	 */
	public function addSelectQuery(Criteria $subQueryCriteria, $alias, $addAliasAndSelectColumns = false)
	{
	...
	{
		$join = $this->joins[$relation];
		call_user_func(array($join->getTableMap()->getPeerClassname(), 'addSelectColumns'), $this, $join->getRightTableAlias());
		...
		if ($subQueryCriteria) {
			if ($tableMap->hasColumnByPhpName($phpName)) {
				...
				if ($failSilently) {
					...
					throw new PropelException('Unknown column ' . $phpName . ' in the subQuery with alias ' . $class);
				}
			}
		// not a subQuery
		} elseif ($tableMap->hasColumnByPhpName($phpName)) {
			$column = $tableMap->getColumnByPhpName($phpName);
