...
class Join
{
	...
	// indication if the right parts of the join conditions are values or fields
	protected $rightIsValue = array();
	 ...
	 * @param bool   $rightIsValue A Boolean indication the right side contains a value that needs to be escaped
	 */
	public function addCondition($left, $right, $operator = self::EQUAL, $rightIsValue = false)
	{
		$this->left[] = $this->validatedFieldName($left);
		$this->right[] = $rightIsValue ? $right : $this->validatedFieldName($right);
		$this->rightIsValue[] = $rightIsValue;
		$this->operator[] = $operator; // TODO: probably validate operator as well, else you can inject there.
		$this->count++;
	}
	
	/**
	 * Tests if the fieldName is valid (does not contain hidden values like 'OR 1=1')
	 * 
	 * @param string $fieldName
	 * @return string $fieldName  the trimmed and validated fieldName
	 */
	public function validatedFieldName($fieldName)
	{
		$fieldName = trim($fieldName);
		
		if (!preg_match('/^\w[.\w]*$/', $fieldName)) {
			throw new PropelException('Invalid fieldname: '.$fieldName.'. if you want to add constraints please use CriterionJoin.');
		}
	...
	public function getConditions()
	{
	  ...
	  for ($i=0; $i < $this->count; $i++) { 
	    $conditions[] = array(
	      ...
	      'rightIsValue' => $this->getRightIsValue($i)
	    );
	  }
	  return $conditions;
	}
	...
	public function getRightIsValue($index = 0)
	{
		return $this->rightIsValue[$index];
	}	
